...
const main = () => {
  ...
  function move() {
    ...
    // arrow right
      ...
      if (currCount + 1 > cols) {
        ...
        currChip.css(
          "left",
          firstRightIncrement + (currCount - 1) * slideIncrement
        );
      }
    }
    // spacebar
  ...
  function winner(row, col, player, num) {
    ...
    if (
      horizontal(row, col, player, 0, true) >= magicNum ||
      vertical(row, col, player, 0, true) >= magicNum ||
      diagonal(row, col, player, 0, true, true) >= magicNum ||
      diagonal(row, col, player, 0, true, false) >= magicNum
    ) {
  ...
  function diagonal(row, col, player, num, positive, isRightDown) {
    ...
    if (!isForward && set !== winningNum) {
      ...
      } else if (num === 0 && set <= winningNum - 1) {
        // -1 bc very first element already included
        sectionWon = [];
      }
    }
